(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("1863e901-3363-4c19-93ac-db7ae7af88ef");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '1863e901-3363-4c19-93ac-db7ae7af88ef' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.3.min.js": "dM3QQsP+wXdHg42wTqW85BjZQdLNNIXqlPw/BgKoExPmTG7ZLML4EGqLMfqHT6ON", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.3.min.js": "8x57I4YuIfu8XyZfFo0XVr2WAT8EK4rh/uDe3wF7YuW2FNUSNEpJbsPaB1nJ2fz2", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.3.min.js": "3QTqdz9LyAm2i0sG5XTePsHec3UHWwVsrOL68SYRoAXsafvfAyqtQ+h440+qIBhS"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"407e3a4a-73a0-499d-9c06-b060a5b02b4c":{"defs":[],"roots":{"references":[{"attributes":{},"id":"15837","type":"AllLabels"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"15822","type":"Line"},{"attributes":{},"id":"15834","type":"AllLabels"},{"attributes":{"toolbars":[{"id":"15805"}],"tools":[{"id":"15795"},{"id":"15796"},{"id":"15797"},{"id":"15798"},{"id":"15799"},{"id":"15800"},{"id":"15801"},{"id":"15802"}]},"id":"15849","type":"ProxyToolbar"},{"attributes":{"active_multi":null,"tools":[{"id":"15795"},{"id":"15796"},{"id":"15797"},{"id":"15798"},{"id":"15799"},{"id":"15800"},{"id":"15801"},{"id":"15802"}]},"id":"15805","type":"Toolbar"},{"attributes":{},"id":"15844","type":"UnionRenderers"},{"attributes":{},"id":"15800","type":"UndoTool"},{"attributes":{"overlay":{"id":"15804"}},"id":"15799","type":"LassoSelectTool"},{"attributes":{"source":{"id":"15814"}},"id":"15818","type":"CDSView"},{"attributes":{"formatter":{"id":"15836"},"major_label_policy":{"id":"15837"},"ticker":{"id":"15788"}},"id":"15787","type":"LinearAxis"},{"attributes":{"callback":null},"id":"15802","type":"HoverTool"},{"attributes":{"data_source":{"id":"15825"},"glyph":{"id":"15826"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15827"},"view":{"id":"15829"}},"id":"15828","type":"GlyphRenderer"},{"attributes":{"children":[{"id":"15850"},{"id":"15848"}]},"id":"15851","type":"Column"},{"attributes":{"source":{"id":"15820"}},"id":"15824","type":"CDSView"},{"attributes":{"toolbar":{"id":"15849"},"toolbar_location":"above"},"id":"15850","type":"ToolbarBox"},{"attributes":{},"id":"15840","type":"UnionRenderers"},{"attributes":{"axis":{"id":"15787"},"ticker":null},"id":"15790","type":"Grid"},{"attributes":{"text":"y / y","text_font_size":"15pt"},"id":"15830","type":"Title"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"15839"},"selection_policy":{"id":"15840"}},"id":"15814","type":"ColumnDataSource"},{"attributes":{"formatter":{"id":"15833"},"major_label_policy":{"id":"15834"},"ticker":{"id":"15792"}},"id":"15791","type":"LinearAxis"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15815","type":"Line"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"15821","type":"Line"},{"attributes":{"data_source":{"id":"15814"},"glyph":{"id":"15815"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15816"},"view":{"id":"15818"}},"id":"15817","type":"GlyphRenderer"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15816","type":"Line"},{"attributes":{},"id":"15785","type":"LinearScale"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"15804","type":"PolyAnnotation"},{"attributes":{"axis":{"id":"15791"},"dimension":1,"ticker":null},"id":"15794","type":"Grid"},{"attributes":{},"id":"15833","type":"BasicTickFormatter"},{"attributes":{},"id":"15796","type":"PanTool"},{"attributes":{"children":[[{"id":"15778"},0,0]]},"id":"15848","type":"GridBox"},{"attributes":{"data_source":{"id":"15820"},"glyph":{"id":"15821"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15822"},"view":{"id":"15824"}},"id":"15823","type":"GlyphRenderer"},{"attributes":{},"id":"15783","type":"LinearScale"},{"attributes":{},"id":"15841","type":"Selection"},{"attributes":{"bottom":1.459677659268402,"fill_alpha":0.1,"fill_color":"#2a2eec","top":0.6850827603275131},"id":"15819","type":"BoxAnnotation"},{"attributes":{"source":{"id":"15825"}},"id":"15829","type":"CDSView"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"15803","type":"BoxAnnotation"},{"attributes":{},"id":"15781","type":"DataRange1d"},{"attributes":{},"id":"15836","type":"BasicTickFormatter"},{"attributes":{"below":[{"id":"15787"}],"center":[{"id":"15790"},{"id":"15794"},{"id":"15819"}],"height":500,"left":[{"id":"15791"}],"output_backend":"webgl","renderers":[{"id":"15817"},{"id":"15823"},{"id":"15828"}],"title":{"id":"15830"},"toolbar":{"id":"15805"},"toolbar_location":null,"width":500,"x_range":{"id":"15779"},"x_scale":{"id":"15783"},"y_range":{"id":"15781"},"y_scale":{"id":"15785"}},"id":"15778","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"15792","type":"BasicTicker"},{"attributes":{},"id":"15842","type":"UnionRenderers"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"15841"},"selection_policy":{"id":"15842"}},"id":"15820","type":"ColumnDataSource"},{"attributes":{},"id":"15798","type":"WheelZoomTool"},{"attributes":{},"id":"15843","type":"Selection"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15827","type":"Line"},{"attributes":{"overlay":{"id":"15803"}},"id":"15797","type":"BoxZoomTool"},{"attributes":{"data":{},"selected":{"id":"15843"},"selection_policy":{"id":"15844"}},"id":"15825","type":"ColumnDataSource"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15826","type":"Line"},{"attributes":{},"id":"15801","type":"SaveTool"},{"attributes":{},"id":"15839","type":"Selection"},{"attributes":{},"id":"15779","type":"DataRange1d"},{"attributes":{},"id":"15795","type":"ResetTool"},{"attributes":{},"id":"15788","type":"BasicTicker"}],"root_ids":["15851"]},"title":"Bokeh Application","version":"2.3.3"}}';
                  var render_items = [{"docid":"407e3a4a-73a0-499d-9c06-b060a5b02b4c","root_ids":["15851"],"roots":{"15851":"1863e901-3363-4c19-93ac-db7ae7af88ef"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();